...
if config['network']['request_interval_time'] != None:
...
def download_chapter(chapter_info, progress, task_id, chapter_count):
    index, (chapter_name, chapter_url) = chapter_info
    try:
        # 根据下载模式更新不同的进度条描述
        if multi_threading:
            # 多线程模式：显示索引但不显示章节名
            progress.update(task_id, description=f"[cyan]下载中[{index+1}/{chapter_count}]: ")
        else:
            # 单线程模式：显示索引和章节名前20字符
            short_name = chapter_name[:20] + '...' if len(chapter_name) > 20 else chapter_name
            progress.update(task_id, description=f"[cyan]下载中[{index+1}/{chapter_count}]: {short_name}")
        ...
        if chapter_response is None:
            console_log(f"获取章节内容失败: {chapter_name}", level='error')
            return (index, chapter_name, "", False, "请求失败")
        ...
        if len(chapter_content_div) == 0:
            console_log(f"找不到符合规则的章节内容区域: {chapter_name}", level='error')
            return (index, chapter_name, "", False, "内容区域未找到")
        ...
        for chapter_content_div_only in chapter_content_div:
            ...
            if len(chapter_content_p_list) == 0:
                console_log(f"找不到符合规则的章节内容段落: {chapter_name}", level='warn')
                continue
        ...
        if chapter_content == '':
            console_log(f"章节内容为空: {chapter_name}", level='warn')
            return (index, chapter_name, "", True, "内容为空")
        else:
            # 净化章节内容
            chapter_content = purify_content(chapter_content)
            return (index, chapter_name, chapter_content, True, "下载成功")
        ...
        return (index, chapter_name, "", False, error_msg)
...
def main():
    if debug:
        ...
        console_log(f"多线程模式: {multi_threading}", level='debug')
        console_log(f"线程数: {thread_count}", level='debug')
    ...
    def txt_writer():
        ...
        while completed_counter < chapter_count:
            # 从队列中获取结果
            index, chapter_name, chapter_content, success, message = write_queue.get()
            
            # 存储结果
            results[index] = (chapter_name, chapter_content, success, message)
            
            # 按顺序处理结果
            while next_index in results:
                chapter_name, chapter_content, success, message = results.pop(next_index)
                ...
                if output_format == 'txt' and success and chapter_content:
                    try:
                        ...
                        console_log(f"写入章节失败: {chapter_name} - {e}", level='error')
    ...
    with progress:
        if multi_threading:
            ...
            for future in as_completed(futures):
                index, chapter_name, chapter_content, success, message = future.result()
                ...
                if output_format == 'epub' and success and chapter_content:
                    # 存储章节内容，稍后按顺序处理
                    epub_chapters[index] = (chapter_name, chapter_content)
                
                # TXT将结果放入队列
                elif output_format == 'txt':
                    write_queue.put((index, chapter_name, chapter_content, success, message))
            ...
            for i, (chapter_name, chapter_url) in enumerate(chapter_list):
                result = download_chapter(
                    (i, (chapter_name, chapter_url)), 
                    progress, 
                    task_id,
                    chapter_count
                )
                index, chapter_name, chapter_content, success, message = result
                ...
                if output_format == 'txt' and success and chapter_content:
                    ...
                    # 存储章节内容
                    epub_chapters[i] = (chapter_name, chapter_content)
    ...
    if output_format == 'epub':
        ...
        for index in sorted(epub_chapters.keys()):
            chapter_name, chapter_content = epub_chapters[index]
            ...
            epub_book_chapter_listtoc.append((chapter_id, chapter_name))
            # 添加到书籍
